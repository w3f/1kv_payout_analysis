...
for(i in 1:length(x)) {
  ...
  # Before throwing out the validators of the empty stakers, we need them to calculate the sum of all era_points
  validators_all <- validators
  ...
  df_output$our_payoff <- (df_output$era_points / sum(subset(validators_all, active==1)$era_points)) * validator_rewards * ((100-df_output$commission)/100) * (df_output$our_stake / df_output$total_stake)
  # Calculate overall payoff per validator
  df_output$total_payoff <- (df_output$era_points / sum(subset(validators_all, active==1)$era_points)) * validator_rewards
